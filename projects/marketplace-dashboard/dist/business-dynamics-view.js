(function () {
  ...
  function executiveKpisMarkup(model, currentDay) {
    ...
    var knownYesterday = data.yesterdayLatest && finite(data.yesterdayLatest.value);
    var priorFrom = knownYesterday ? formatTime(data.yesterdayLatest.from, model.timezone) : null;
    var priorTo = knownYesterday ? formatTime(data.yesterdayLatest.to, model.timezone) : null;
    var priorCutoff = priorFrom === priorTo ? 'Срез ' + priorFrom : 'Срезы ' + priorFrom + '–' + priorTo;
    var rows = [
      ...
      { label: finite(data.yesterdaySameTime) ? (currentDay ? 'Вчера к этому времени' : 'Предыдущий день к этому времени') : finite(data.yesterdayFullDay) ? (currentDay ? 'Вчера за день' : 'Предыдущий день за день') : knownYesterday ? (currentDay ? 'Вчера · известная часть' : 'Предыдущий день · известная часть') : (currentDay ? 'Вчера к этому времени' : 'Предыдущий день к этому времени'), value: finite(data.yesterdaySameTime) ? data.yesterdaySameTime : finite(data.yesterdayFullDay) ? data.yesterdayFullDay : knownYesterday ? data.yesterdayLatest.value : null, unit: 'rub', note: finite(data.yesterdaySameTime) ? 'Одинаковый момент МСК' : finite(data.yesterdayFullDay) ? 'Полный подтверждённый итог; нет сопоставимого среза по времени' : knownYesterday ? priorCutoff + ' МСК · точного сравнения нет' : 'Одинаковый момент МСК', kind: 'yesterday', trend: miniTrend(history, 'Динамика предыдущего дня', 'muted') },
