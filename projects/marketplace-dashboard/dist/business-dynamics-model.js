(function(root){'use strict';
 ...
 function combined(stores,date,offset,metric,{exact=false,fullDay=false}={}){
  ...
  return {value:valueOf(components,metric),...components,complete,rows,ozon:valueOf(add(valid.filter(row=>row.store.market==='Ozon').map(row=>row.point)),metric),wb:valueOf(add(valid.filter(row=>row.store.market==='WB').map(row=>row.point)),metric)};
 }
 ...
 function build(payload,{date=payload?.period?.to,metric='orderedRevenue',selectedIds=[''],now=Date.now()}={}){
  ...
  for(const t of offsets){const row=combined(stores,date,t,metric),exact=combined(stores,date,t,metric,{exact:true}),previous=combined(stores,shift(date,-1),t,metric,{exact:true,fullDay:true}),comparable=exact.complete&&previous.complete&&sameBasis(stores,date,shift(date,-1));today.push({at:iso(base+t),cumulative:row.value,complete:exact.complete,basis:allEvents?'order-time':'observation',last15:intervalValue(stores,date,t-STEP,t,metric),orders:row.orderCount,units:row.orderedUnits,avgCheck:row.orderCount>0&&finite(row.orderedRevenue)?row.orderedRevenue/row.orderCount:null,ozon:row.ozon,wb:row.wb,yesterday:comparable?previous.value:null,vsYesterdayPct:comparable?percent(row.value,previous.value):null});}
  // A staggered aggregate is explicitly a known subtotal, not an exact-time total.
  const value=today.at(-1)?.cumulative??null;
