...
function createPostgresArchiveRepository({ pool, history = null, schema = 'pult_history', now = Date.now } = {}) {
  ...
  async function add({ sourceFile, sourceMtime, raw, capturedAt } = {}) {
    ...
    const stamp = raw.length + ':' + sourceMtime;
    const factsStatus = history && FACT_FILES.test(sourceFile) ? 'pending' : 'not_applicable';
    const client = await connectDatabase(pool);
    try {
      await queryDatabase(client, 'BEGIN');
      await acquireMutationFence(client);
      const inserted = await queryDatabase(client, `INSERT INTO ${table('archive_versions')}(source_file,content_hash,captured_at,captured_at_text,source_mtime,source_bytes,archive_bytes,object_path,facts_status,archive_payload,archive_gzip_hash) VALUES($1,$2,$3::text::timestamptz,$3::text,$4,$5,$6,NULL,$7,$8,$9) ON CONFLICT(source_file,content_hash) DO NOTHING RETURNING source_file`, [sourceFile, contentHash, capturedAt, sourceMtime, raw.length, archivePayload.length, factsStatus, archivePayload, gzipHash]);
      if (!inserted.rowCount) {
        const existing = await queryDatabase(client, `SELECT content_hash,source_bytes,archive_payload,archive_gzip_hash FROM ${table('archive_versions')} WHERE source_file=$1 AND content_hash=$2 FOR UPDATE`, [sourceFile, contentHash]);
        if (!existing.rowCount || !unpackAndVerify(existing.rows[0]).equals(raw)) throw corruptArchive();
      }
      const latestResult = await queryDatabase(client, `INSERT INTO ${table('archive_latest')} AS current(source_file,stamp,content_hash) VALUES($1,$2,$3) ON CONFLICT(source_file) DO UPDATE SET stamp=EXCLUDED.stamp,content_hash=EXCLUDED.content_hash WHERE $4::double precision>=substring(current.stamp from position(':' in current.stamp)+1)::double precision RETURNING source_file`, [sourceFile, stamp, contentHash, sourceMtime]);
      await queryDatabase(client, 'COMMIT', undefined, { commitOutcome: true });
      return { changed: !!inserted.rowCount, latestUpdated: !!latestResult.rowCount, contentHash, gzipHash: gzipHash.toString('hex'), factsStatus: inserted.rowCount ? factsStatus : undefined };
    } catch (error) {
  ...
  async function processPendingFacts({ limit = 100 } = {}) {
    ...
    while (imported < limit) {
      ...
      try {
        await queryDatabase(client, 'BEGIN');
        await acquireMutationFence(client);
        const selected = await queryDatabase(client, `SELECT source_file,content_hash,captured_at_text,source_bytes,archive_payload,archive_gzip_hash FROM ${table('archive_versions')} WHERE facts_status='pending' ORDER BY captured_at,source_file,content_hash FOR UPDATE SKIP LOCKED LIMIT 1`);
        if (!selected.rowCount) {
          await queryDatabase(client, 'COMMIT');
          break;
        }
        const row = selected.rows[0], raw = unpackAndVerify(row);
        let data;
        try { data = JSON.parse(raw.toString('utf8').replace(/^\uFEFF/u, '')); } catch { throw factImportError(); }
        try { await history.ingestInTransaction({ sourceFile: row.source_file, contentHash: row.content_hash, data, capturedAt: row.captured_at_text }, client); } catch { throw factImportError(); }
  ...
  return { add, latest, processPendingFacts, status };
}

module.exports = { createPostgresArchiveRepository, ArchiveRepositoryError, MAX_SOURCE_BYTES };
