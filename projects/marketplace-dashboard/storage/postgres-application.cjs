...
const {createWbHistoryBackfill}=require('./acquisition/postgres-wb-history-backfill.cjs');
const {createWbHistoryScheduler}=require('./acquisition/postgres-wb-history-scheduler.cjs');
...
async function createPostgresApplication({pool,readPool=pool,analyticsPool=pool,ozonHttpPool=pool,ozonApiFactory=createPostgresOzonApi,staticDir,staticFiles,stateSchema='pult',marketSchema='pult_market',historySchema='pult_history',partnerPort=4319,protect=windowsProtect,fetchFn=globalThis.fetch,now=()=>Date.now(),releaseNotes,loadReleaseNotes,extractFile=extractor(),verifyProviders}={}){
 ...
 // Full source reads have their own bounded queue; UI connections remain available
 // for stores and business dynamics. Reuse the same repository/transaction code.
 const analyticsLive=analyticsPool===pool?live:analyticsPool===readPool?readLive:await createLiveComposition({pool:analyticsPool,stateSchema,marketSchema});
 const {stateStore,marketRepository,scheduler,marketWriter}=live,sourceProviders=analyticsLive.sourceProviders,storesRepository=createStores({stateStore}),productTypes=createProductTypes({stateStore}),intraday=createIntraday({stateStore}),categoryCapture=createCategoryCapture({stateStore});
 ...
 const trueStats=createTrueStats({stateStore,protect,transport:fetchFn,now,getProducts:sourceProviders.getProducts}),analytics=createAnalytics({pool:analyticsPool,stateSchema,storesRepository,productTypes,supplierPortals,trueStats,now,sourceProviders});categorySalesReader=analytics.categorySales;
 ...
 const wbHistoryScheduler=createWbHistoryScheduler({pool,storesRepository,backfill:createWbHistoryBackfill({pool,storesRepository,decrypt,history:historyFacts,fetchFn,now}),now});
 ...
 const businessDynamics=createBusinessDynamics({repository:createBusinessDynamicsRepository({pool:readPool,writePool:pool}),storesRepository:createStores({stateStore:readLive.stateStore}),now});
