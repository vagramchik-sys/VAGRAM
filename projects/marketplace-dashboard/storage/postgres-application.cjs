...
const {createOzonApi,createWbApi}=require('./acquisition/postgres-marketplace-transport.cjs');
...
async function createPostgresApplication({pool,readPool=pool,staticDir,staticFiles,stateSchema='pult',marketSchema='pult_market',historySchema='pult_history',partnerPort=4319,protect=windowsProtect,fetchFn=globalThis.fetch,now=()=>Date.now(),releaseNotes=require('../release-notes.json'),extractFile=extractor(),verifyProviders}={}){
 ...
 const ozonApi=createOzonApi({fetchFn}),wbApi=createWbApi({fetchFn}),decrypt=value=>protect(value,true),ledgerRefresh=createPostgresLiveLedgerRefresh({liveSources:live.sources}),market=createMarketAcquisition({storesRepository,marketRepository,marketWriter,ledgerRefresh,ozonCollector:createOzonSnapshotCollector({fetchFn,now:()=>new Date(now())}),wbCollector:createWbSnapshotCollector({fetchFn,now:()=>new Date(now())}),decrypt}),costsPrices=createCostsPricesAcquisition({stateStore,storesRepository,decrypt,ozonApi,fetchFn}),insights=createPostgresInsights({stateStore,storesRepository,marketRepository,ledgerRefresh,decrypt,ordersCollector:createOzonOrdersCollector({api:ozonApi,now:()=>new Date(now())}),funnelCollector:createOzonFunnelCollector({api:ozonApi,now})}),wbOrders=createPostgresWbOrders({stateStore,storesRepository,decrypt,fetchFn,now});
 const acquisition={market,costsPrices,insights,wbOrders,derivedCapture},dispatchers=createAcquisitionDispatchers({stateStore,...acquisition}),runner=createSchedulerRunner({scheduler,dispatchers,runnerId:`runtime-${process.pid}`,uuid:crypto.randomUUID,now:()=>new Date(now())}),cadence=createCadenceProducer({scheduler,storesRepository,stateStore,readMarketDocument:live.readMarketDocument,readDocument:live.readDocument,buildDerivedPayload:derivedInputs.build,now:()=>new Date(now())}),scheduleView=schedules(scheduler,runner,cadence);
