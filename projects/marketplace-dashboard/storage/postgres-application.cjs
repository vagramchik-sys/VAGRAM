...
async function createPostgresApplication({pool,readPool=pool,ozonHttpPool=pool,ozonApiFactory=createPostgresOzonApi,staticDir,staticFiles,stateSchema='pult',marketSchema='pult_market',historySchema='pult_history',partnerPort=4319,protect=windowsProtect,fetchFn=globalThis.fetch,now=()=>Date.now(),releaseNotes,loadReleaseNotes,extractFile=extractor(),verifyProviders}={}){
 ...
 const moduleRegistry=createModuleRegistry({
  development:process.env.PULT_DEV_MODULES==='1',
  disabled:String(process.env.PULT_MODULES_DISABLED||'').split(',').map(value=>value.trim()).filter(Boolean),
  ctx:{db:readPool,scheduler,marketplaces:sourceProviders,metrics:requestMetrics,
   logger:Object.freeze({error(value){const event=value&&typeof value==='object'?value:{};console.error(JSON.stringify({module:String(event.module||'unknown').slice(0,60),route:String(event.route||'unknown').slice(0,100),operation:String(event.operation||'unknown').slice(0,60),code:String(event.code||'MODULE_ERROR').slice(0,60),durationMs:Number.isFinite(event.durationMs)?event.durationMs:Number.isFinite(event.duration)?event.duration:null}))}})}
 });
