...
const { postgresHistoryInventory, summarizeHistoryInventory } = require('./postgres-history-parity.cjs');
...
const KINDS = new Set(['history.ingest', 'archive.version-add', 'archive.process-exact-pending-version']);
...
function parseEnvelope(bytes, domain) {
  let value; try { value=JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(bytes)); if(!encodeJson(value,320*1024*1024).equals(bytes)) throw Error('canonical'); } catch { fail('JOURNAL_INVALID','Domain command envelope is invalid'); }
  if(value?.schemaVersion!==1||!KINDS.has(value.kind)||domain==='history'&&value.kind!=='history.ingest'||domain==='archive'&&!value.kind.startsWith('archive.')) fail('UNSUPPORTED_DOMAIN','Unsupported domain command');
  for(const name of value.kind==='history.ingest'?['rawBase64','contentHash']:value.kind==='archive.version-add'?['rawBase64','proposedGzipBase64','contentHash','proposedGzipHash']:['rawBase64','gzipBase64','contentHash','gzipHash']) if(typeof value[name]!=='string') fail('JOURNAL_INVALID','Domain evidence is incomplete');
  const decode=name=>{const result=Buffer.from(value[name],'base64');if(result.toString('base64')!==value[name])fail('JOURNAL_INVALID','Domain evidence base64 is invalid');return result},raw=decode('rawBase64');
  if(digest(raw)!==value.contentHash)fail('JOURNAL_INVALID','Domain raw evidence hash differs');
  if(value.kind.startsWith('archive.')){const gzipName=value.kind==='archive.version-add'?'proposedGzipBase64':'gzipBase64',hashName=value.kind==='archive.version-add'?'proposedGzipHash':'gzipHash',gzip=decode(gzipName);let unpacked;try{unpacked=gunzipSync(gzip,{maxOutputLength:64*1024*1024})}catch{fail('JOURNAL_INVALID','Archive gzip evidence is corrupt')}if(digest(gzip)!==value[hashName]||!unpacked.equals(raw))fail('JOURNAL_INVALID','Archive gzip evidence differs from raw')}
  return value;
}
...
async function exportRuntimeJournal({ pool, checkpointSequence, destinationDir }={}) {
  ...
  try{let upper=baseline,commands=[],baselineDocuments=[],expectedHistoryBusinessSummary=null;const blobs=new Map();await withWriteFence({pool,timeoutMs:30000},async client=>{await client.query('BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY');try{
    ...
    try{for(;;){const rows=await client.query('FETCH FORWARD 16 FROM pult_runtime_journal');if(!rows.rows.length)break;for(const r of rows.rows){const beforeRevision=String(r.before_revision),afterRevision=String(r.after_revision),beforeBytes=stateBlob(r.before_content,r.before_sha256,r.before_deleted,beforeRevision),afterBytes=stateBlob(r.after_content,r.after_sha256,r.after_deleted,afterRevision);const before=beforeBytes?await writeBlob(root,beforeBytes,blobs):null,after=afterBytes?await writeBlob(root,afterBytes,blobs):null;
      ...
      else {domain=String(r.logical_key).split('/')[0];if(!DOMAIN_MEDIA[domain]||r.media_type!==DOMAIN_MEDIA[domain]||r.operation!=='write'||!afterBytes||r.result_json==null)fail('UNSUPPORTED_DOMAIN','Unsupported SQL domain command');type=domain;envelope=parseEnvelope(afterBytes,domain);try{result=JSON.parse(encodeJson(r.result_json,64*1024).toString())}catch{fail('JOURNAL_INVALID','Invalid durable command result')}}
    ...
    }}}finally{await client.query('CLOSE pult_runtime_journal').catch(()=>{})}validateCommands(commands,baseline,upper);expectedHistoryBusinessSummary=summarizeHistoryInventory(await postgresHistoryInventory(client));await client.query('COMMIT');}catch(e){await client.query('ROLLBACK').catch(()=>{});throw e}});
    const manifest={schemaVersion:1,status:'complete',scope:'documents-history-archive',baselineSequence:baseline,upperSequence:upper,baselineDocuments,commands,blobs:[...blobs.values()].sort((a,b)=>a.sha256.localeCompare(b.sha256)),expectedHistoryBusinessSummary,rollbackReady:false};const bytes=await writeJson(path.join(root,MANIFEST_FILE),manifest);await fs.unlink(incomplete);await writeJson(path.join(root,COMPLETE_MARKER),{schemaVersion:1,manifest:MANIFEST_FILE,manifestSha256:digest(bytes)});await verifyRuntimeJournal(root);return manifest;
...
async function verifyRuntimeJournal(directory){const root=await rootDirectory(directory);if(await fs.access(path.join(root,INCOMPLETE_MARKER)).then(()=>true,()=>false))fail('JOURNAL_INCOMPLETE','Runtime journal incomplete');let manifest,bytes,marker;try{bytes=await fs.readFile(await safeFile(root,MANIFEST_FILE));marker=JSON.parse(await fs.readFile(await safeFile(root,COMPLETE_MARKER),'utf8'));manifest=JSON.parse(bytes)}catch(e){if(e instanceof PostgresRuntimeJournalError)throw e;fail('JOURNAL_INVALID','Runtime journal metadata invalid')}
  ...
  for(const c of manifest.commands){for(const s of [c.before,c.after]){if(!s||![true,false,null].includes(s.deleted)||s.blob!==null&&!same(s.blob,declared.get(s.blob?.sha256))||s.deleted===false&&s.blob===null||s.deleted!==false&&s.blob!==null)fail('JOURNAL_INVALID','Command blob invalid')}if(c.type==='document'){portable(c.sourcePath);if(c.envelope!==null||c.result!==null)fail('JOURNAL_INVALID','Document command shape invalid')}else{if(!DOMAIN_MEDIA[c.type]||c.sourcePath!==null||c.operation!=='write'||c.mediaType!==DOMAIN_MEDIA[c.type]||c.after.deleted!==false||!c.after.blob||!same(parseEnvelope(await fs.readFile(await safeFile(root,c.after.blob.path)),c.type),c.envelope)||c.result==null)fail('JOURNAL_INVALID','Domain command shape invalid')}}validateCommands(manifest.commands,manifest.baselineSequence,manifest.upperSequence);
