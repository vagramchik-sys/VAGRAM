...
module.exports=function createPostgresReportRoutes({storesRepository,sourceProviders,supplierPortals,productTypes,intraday,orderCategoryState,trueStats,schedules,now=()=>Date.now()}={}){
 ...
 async function economicsCompare(params){const {selected}=await selectedOzon(params,400),report=insightsPure.report(await reportInputs(selected),{from:params.get('from'),to:params.get('to')});return trueStats.compare({period:report.current,stores:selected.map(store=>({id:store.id,name:store.name})),localEconomics:report.economics})}
 async function handle(req,res,url){if(req.method!=='GET')return false;const routes={'/api/insights':insights,'/api/insights/sources':insightSources,'/api/order-categories':orderCategories,'/api/wb/orders':wbOrders,'/api/economics/compare':economicsCompare},method=routes[url.pathname];if(!method)return false;try{writeJson(res,200,await method(url.searchParams))}catch(error){const safe=error instanceof ReportRouteError;writeJson(res,safe?error.status:400,{error:safe?error.message:'Не удалось сформировать отчёт.'})}return true}
