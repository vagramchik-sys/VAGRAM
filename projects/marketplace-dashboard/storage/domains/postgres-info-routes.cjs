...
module.exports=function createPostgresInfoRoutes({stateStore,releaseNotes,loadReleaseNotes,storesRepository,scheduler,readSourceMetadata,now=()=>Date.now()}={}){
 if(!stateStore||(!releaseNotes&&typeof loadReleaseNotes!=='function'))throw new TypeError('stateStore and release notes source are required');let currentReleaseNotes=releaseNotes&&typeof releaseNotes==='object'&&!Array.isArray(releaseNotes)?structuredClone(releaseNotes):null;const repo=createJsonDocumentRepository({stateStore,logicalKey:sourceKey('company-impact.json'),sourcePath:'company-impact.json',validate:validImpact});
 ...
 async function changes(){if(typeof loadReleaseNotes==='function')try{const next=await loadReleaseNotes();if(!next||typeof next!=='object'||Array.isArray(next)||!Array.isArray(next.entries))throw Error('INVALID_RELEASE_NOTES');currentReleaseNotes=structuredClone(next)}catch(error){if(!currentReleaseNotes)throw error}if(!currentReleaseNotes)throw Error('RELEASE_NOTES_UNAVAILABLE');return structuredClone(currentReleaseNotes)}
 async function handle(req,res,url){if(!['/api/impact','/api/changes','/api/data-updates'].includes(url.pathname))return false;if(req.method!=='GET'){reply(res,405,{error:'Метод не поддерживается.'});return true}if(url.pathname==='/api/data-updates'){try{reply(res,200,await dataUpdates())}catch{reply(res,503,{error:'Статусы обновлений временно недоступны.'})}return true}if(url.pathname==='/api/changes'){try{reply(res,200,await changes())}catch{reply(res,503,{error:'Журнал изменений временно недоступен.'})}}else{const row=await repo.read();reply(res,200,row&&!row.deleted?{donatedRub:row.value.donatedRub,childrenHomes:row.value.childrenHomes,updatedAt:row.value.updatedAt,source:'owner'}:null)}return true}
