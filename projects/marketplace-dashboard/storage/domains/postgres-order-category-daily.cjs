...
module.exports = function createPostgresOrderCategoryDaily({ productTypes, getCatalogs, getSnapshots, getInsights, getWbOrders, getStores = async () => ({}), now = () => Date.now() } = {}) {
  ...
  async function buildReport(options) {
    const [registry, catalogsRaw, snapshots, insightsRaw, wbRaw, stores] = await Promise.all([productTypes.read(), getCatalogs(), getSnapshots({ from: options.from, to: options.to }), getInsights(), getWbOrders(), getStores()]);
  ...
  function read(options = {}) {
    const key = JSON.stringify([options.from, options.to, options.market || 'all', options.store || options.storeId || null, options.now, options.classifiedAt]);
    if (pending.has(key)) return pending.get(key);
    const request = buildReport(options).finally(() => { if (pending.get(key) === request) pending.delete(key); });
