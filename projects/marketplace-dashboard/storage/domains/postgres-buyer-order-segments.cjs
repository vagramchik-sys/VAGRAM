...
function create({getStores,getSnapshot}={}){
 if(typeof getStores!=='function'||typeof getSnapshot!=='function')throw Error('Не настроен SQL-источник сегментов заказов');
 async function read({from,to,market='all',storeId}={}){
  ...
  const snapshot=await getSnapshot({from,to}),matches=value=>(market==='all'||value.market===market)&&(!storeId||value.storeId===storeId);
  const raw=snapshot&&Array.isArray(snapshot.records)&&Array.isArray(snapshot.report?.coverage?.sources)?buyerSegments.aggregate(snapshot.records.filter(matches),{from,to,sources:snapshot.report.coverage.sources.filter(matches)}):buyerSegments.aggregate([],{from,to,sources:[]});
