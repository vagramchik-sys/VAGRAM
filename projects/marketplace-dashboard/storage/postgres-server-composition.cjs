...
function createPostgresServerComposition({ pool, readPool = pool, stateStore, stateSchema = 'pult', marketSchema = 'pult_market', protect, transport, now, ownerAdapters, scheduler, schedulerRunner, cadenceProducer, derivedCaptureRunner, acquisition, staticDir, staticFiles, moduleAssets, sourceProviders, trueStats, analytics, verifyProviders, capabilities = {}, additionalHandlers = [], handlerFactories = {}, background = [], readAdapters, runtimeFactory = createPostgresRuntime } = {}) {
  ...
  const runtime = runtimeFactory({ pool, readPool, stateStore, stateSchema, marketSchema, scheduler, schedulerRunner, ownerAdapters, acquisition, otherHandlers: [analyticsHandler, ...additionalHandlers], handlerFactories: factories, background: tasks, staticDir, staticFiles, moduleAssets, runtimeReadiness: compositionGate, readAdapters });
