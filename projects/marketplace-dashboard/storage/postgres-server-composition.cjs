...
const ROUTE_CAPABILITIES = Object.freeze({
  ...
  '/api/insights': 'insights-api', '/api/insights/sources': 'insights-api', '/api/insights/refresh': 'insights-api',
...
const BACKGROUND_CAPABILITIES = Object.freeze({ schedulerExecution: 'scheduler-runner', periodicAttempts: 'cadence-producer', marketSnapshots: 'market-acquisition', costsAndPrices: 'costs-prices', orders: 'insights-orders', funnel: 'insights-funnel', intradayCapture: 'intraday-writer', categoryCapture: 'category-writer' });

const REQUIRED = Object.freeze(['core', 'owner', 'workspace', ...Object.values(READ_ROUTES).map(([slot]) => `analytics.${slot}`), 'truestats', 'provider-contracts', 'cadence-producer', 'store-commands', 'insights-api', 'pricing-refresh', 'wb-orders-report', 'order-categories', 'economics-compare', 'impact', 'release-notes', 'finance-documents', 'partner-tools', 'charity-tools']);
...
function createPostgresServerComposition({ pool, stateStore, stateSchema = 'pult', protect, transport, now, ownerAdapters, scheduler, schedulerRunner, acquisition, staticDir, staticFiles, sourceProviders, trueStats, analytics, verifyProviders, capabilities = {}, additionalHandlers = [], background = [], runtimeFactory = createPostgresRuntime } = {}) {
  if (!pool || !stateStore || typeof protect !== 'function' || !object(ownerAdapters) || !Array.isArray(additionalHandlers) || !Array.isArray(background)) throw new TypeError('Complete SQL server composition dependencies are required');
  const storesRepository = createStores({ stateStore });
  sourceProviders ||= createPostgresSourceProviders({ pool, stateSchema });
  ...
  for (const name of ['partner-tools', 'charity-tools']) if (typeof namedHandlers.get(name)?.handle === 'function') available.add(name);
  ...
  const runtime = runtimeFactory({ pool, stateStore, scheduler, schedulerRunner, ownerAdapters, acquisition, otherHandlers: [analyticsHandler, ...additionalHandlers], background, staticDir, staticFiles });
  async function readiness() { const base = await runtime.readiness(); const missing = [...new Set([...(base.missingAdapters || []), ...missingCapabilities])]; if (typeof verifyProviders === 'function') { let check; try { check = await verifyProviders({ storesRepository, sourceProviders, trueStats, analytics }); } catch { check = null; } if (check?.ready !== true) missing.push(...(Array.isArray(check?.missingAdapters) && check.missingAdapters.length ? check.missingAdapters : ['provider-contracts'])); } return { ready: base.ready === true && missing.length === 0, missingAdapters: [...new Set(missing)], passive: false, coverage, backgroundCoverage }; }
...
module.exports = { createPostgresServerComposition, createAnalyticsHandler, ROUTE_CAPABILITIES, BACKGROUND_CAPABILITIES, REQUIRED };
