...
test('SQL summary reads a consistent head and aggregates operations before PostgreSQL transfer',async()=>{
  const metadata=require('../storage/postgres-live-codecs.cjs').encode('data-1.json',source).metadata,statements=[],client={async query(sql,args){statements.push(sql);if(sql.startsWith('BEGIN')||sql==='COMMIT')return{rows:[]};if(sql.includes('FROM pult_live.heads'))return{rows:[{revision:1,metadata,entity_counts:{products:1,stocks:1,operations:3}}]};if(sql.includes("entity_type=ANY"))return{rows:[{entity_type:'products',value:source.products[0]},{entity_type:'stocks',value:source.stocks[0]}]};if(sql.includes('GROUPING SETS'))return{rows:[{kind:'group',name:'Sale',day:null,currency:'RUB',cents:125,records:'1',first_order:0,safe:true},{kind:'group',name:'Return',day:null,currency:'RUB',cents:0,records:'1',first_order:1,safe:true},{kind:'group',name:'WB sale',day:null,currency:'RUB',cents:-235,records:'1',first_order:2,safe:true},{kind:'day',name:null,day:'2026-09-22',currency:'RUB',cents:125,records:'1',first_order:0,safe:true},{kind:'day',name:null,day:'2026-09-21',currency:'RUB',cents:0,records:'1',first_order:1,safe:true},{kind:'day',name:null,day:'2026-09-20',currency:'RUB',cents:-235,records:'1',first_order:2,safe:true},{kind:'total',name:null,day:null,currency:null,cents:-110,records:'3',first_order:0,safe:true}]};throw Error(String(args))},release(){statements.push('release')}};
  ...
  assert.deepEqual(summarize(value),summarize(source));assert.equal(value.operations,undefined);assert.equal(value.operationSummary.operationCount,3);assert.ok(statements.some(sql=>sql.includes('GROUPING SETS')));assert.equal(statements.includes('COMMIT'),true);assert.equal(statements.at(-1),'release');
});

test('SQL summary falls back to projected rows when an operation cannot be aggregated safely',async()=>{
  const metadata=require('../storage/postgres-live-codecs.cjs').encode('data-1.json',source).metadata,client={async query(sql){if(sql.startsWith('BEGIN')||sql==='COMMIT')return{rows:[]};if(sql.includes('FROM pult_live.heads'))return{rows:[{metadata,entity_counts:{products:1,stocks:1,operations:3}}]};if(sql.includes('entity_type=ANY'))return{rows:[{entity_type:'products',value:source.products[0]},{entity_type:'stocks',value:source.stocks[0]}]};if(sql.includes('GROUPING SETS'))return{rows:[{kind:'total',records:'3',safe:false}]};if(sql.includes("entity_type='operations'"))return{rows:source.operations.map(value=>({value}))};throw Error('unexpected')},release(){}};
  const repo=createLiveMarketRepository({liveSources:{record(){},repository:{listRows(){},getHead(){}}},pool:{async connect(){return client}}});
  assert.deepEqual(summarize(await repo.getSummarySnapshot('1')),summarize(source));
});
