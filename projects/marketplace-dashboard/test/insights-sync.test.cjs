...
const row=(date,revenue,units=1)=>({dimensions:[{id:date}],metrics:[revenue,units]});
...
test('today refresh runs every five minutes, preserves history and leaves the 30-minute full schedule intact',async t=>{
 const f=fixture(t);await f.client.sync('1');const original=f.client.read('1');
 assert.equal(f.calls.length,2);assert.equal(f.client.schedule('1').orders.nextAt,'2026-09-18T09:05:00.000Z');assert.equal(f.client.schedule('1').full.nextAt,'2026-09-18T09:30:00.000Z');
 f.advance(299999);f.client.ensure();await settled(f.client);assert.equal(f.calls.length,2);
 ...
 const result=f.client.read('1');assert.deepEqual(result.orders.daily,original.orders.daily);assert.equal(result.orders.historyUpdatedAt,original.orders.updatedAt);assert.equal(result.completedAt,original.completedAt);assert.equal(result.orders.updatedAt,'2026-09-18T09:05:00.000Z');
 f.advance(25*60000);f.client.ensure();await settled(f.client);assert.equal(f.calls.length,5);assert.equal(f.calls[4].payload.date_from,'2026-07-21');
});
...
test('credential failures are persisted before requesting and survive restarts without immediate retry',async t=>{
 ...
 const restarted=create(f.options);restarted.ensure();await tick();assert.equal(f.calls.length,0);assert.equal(restarted.schedule('1').orders.nextAt,'2026-09-18T09:05:00.000Z');
});
...
test('429 retry-after blocks both schedules and manual requests across restart',async t=>{
 const f=fixture(t);await f.client.sync('1');f.advance(5*60000);f.options.api=async()=>{throw Object.assign(Error('Ozon 429'),{status:429,retryAfterMs:10*60000})};
