...
test('check mode verifies actual composition, includes command transport and never starts listeners', async t => {
  ...
  assert.equal(result.checked, true); assert.equal(started, 0); assert.equal(ended, 3); assert.deepEqual(profiles, ['runtime','ui','outbound']);
});
test('runtime drains before pool close and repeated close is harmless', async t => {
  ...
  finishDrain(); await first; await second; assert.deepEqual(order, ['server','outbound','ui','runtime']);
});
