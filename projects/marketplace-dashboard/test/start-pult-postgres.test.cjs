...
test('check mode verifies actual composition, includes command transport and never starts listeners', async t => {
  ...
  const result = await launch({ staticDir, checkOnly: true, poolFactory: async options => { profiles.push(options.profile); return { profile: options.profile, end: async () => { ended++; } }; }, createApplication: async value => {
    ...
    assert.equal(value.ozonHttpPool.profile, 'outbound'); assert.notEqual(value.ozonHttpPool, value.readPool); assert.equal(value.pool.profile, 'runtime'); assert.equal(value.readPool.profile, 'ui'); assert.notEqual(value.pool, value.readPool);
    assert.equal(value.analyticsPool.profile, 'analytics'); assert.notEqual(value.analyticsPool, value.readPool); assert.notEqual(value.analyticsPool, value.pool);
    return { readiness: async () => ({ ready: true, missingAdapters: [] }), start: async () => { started++; } };
  } });
  assert.equal(result.checked, true); assert.equal(started, 0); assert.equal(ended, 4); assert.deepEqual(profiles, ['runtime','ui','analytics','outbound']);
});
test('runtime drains before pool close and repeated close is harmless', async t => {
  ...
  finishDrain(); await first; await second; assert.deepEqual(order, ['server','outbound','analytics','ui','runtime']);
});
