...
test('committed command replay verifies its original revision before external work',async()=>{let touched=false;const api=createMarketAcquisition({storesRepository:{protectedStore:async()=>{touched=true}},marketRepository:{getSnapshot:async()=>{touched=true}},decrypt:async()=>{touched=true},ozonCollector:{collect:async()=>{touched=true}},wbCollector:{collect:async()=>{touched=true}},marketWriter:{readCommand:async()=>({revision:'4',beforeRevision:'0',snapshotId:'snap',exactBytes:Buffer.from('{"completedAt":"2026-09-22T10:00:00Z","sections":{"products":{"ok":true}}}')}),publish:async()=>{touched=true}}});const result=await api.acquire({storeId:'1',expectedRevision:'0',commandId:COMMAND});assert.equal(result.replayed,true);await assert.rejects(api.acquire({storeId:'1',expectedRevision:'1',commandId:COMMAND}),{code:'COMMAND_ID_REUSED'});assert.equal(touched,false);});
