...
test('committed command replay verifies its original revision and repairs ledger without external work',async()=>{let touched=false,ensured=0;const api=createMarketAcquisition({storesRepository:{protectedStore:async()=>{touched=true}},marketRepository:{getSnapshot:async()=>{touched=true}},ledgerRefresh:{ensure:async()=>{ensured++}},decrypt:async()=>{touched=true},ozonCollector:{collect:async()=>{touched=true}},wbCollector:{collect:async()=>{touched=true}},marketWriter:{readCommand:async()=>({revision:'4',beforeRevision:'0',snapshotId:'snap',exactBytes:Buffer.from('{"completedAt":"2026-09-22T10:00:00Z","sections":{"products":{"ok":true}}}')}),publish:async()=>{touched=true}}});const result=await api.acquire({storeId:'1',expectedRevision:'0',commandId:COMMAND});assert.equal(result.replayed,true);assert.equal(ensured,1);await assert.rejects(api.acquire({storeId:'1',expectedRevision:'1',commandId:COMMAND}),{code:'COMMAND_ID_REUSED'});assert.equal(touched,false);});
test('resolve never calls a collector and resumes ledger from an already committed market command',async()=>{const d=dependencies({marketWriter:{readCommand:async()=>({revision:'2',beforeRevision:'1',snapshotId:'snap',exactBytes:Buffer.from('{"clientId":"1","completedAt":"2026-09-22T10:00:00Z","sections":{}}')}),publish:async()=>{throw Error('unused')}}}),result=await createMarketAcquisition(d.values).resolve({storeId:'1',expectedRevision:'1',commandId:COMMAND});assert.equal(result.replayed,true);assert.deepEqual(d.calls.map(row=>row[0]),['ledger'])});
