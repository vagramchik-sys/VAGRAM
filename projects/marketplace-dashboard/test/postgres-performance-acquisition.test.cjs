...
test('statistics period is only the previous completed Moscow day',()=>{assert.deepEqual(defaultPeriod(new Date('2026-09-24T20:59:59.999Z')),{from:'2026-09-23',to:'2026-09-23'});assert.deepEqual(defaultPeriod(new Date('2026-09-24T21:00:00.000Z')),{from:'2026-09-24',to:'2026-09-24'});assert.deepEqual(defaultPeriod(new Date('2026-01-01T00:00:00.000Z')),{from:'2025-12-31',to:'2025-12-31'})});
test('refresh preserves every undocumented bid unit and attributed orders above clicks',async()=>{const f=fixture(),result=await f.api.refresh({storeId:'1',expectedRevision:'0',commandId:COMMAND});assert.equal(result.status,'done');const saved=f.calls.find(row=>row[0]==='commit')[1];assert.equal(saved.products[0].current_bid,null);assert.equal(saved.products[0].competitive_bid,null);assert.equal(saved.products[0].minimum_bid,null);assert.equal(saved.products[0].current_bid_raw,'12500000');assert.equal(saved.products[0].competitive_bid_raw,'13750000');assert.equal(saved.products[0].current_bid_raw_unit,'OZON_CPC_BID_UNSPECIFIED');assert.equal(saved.products[0].minimum_bid_raw,'3.5');assert.equal(saved.products[0].minimum_bid_raw_unit,'OZON_CPC_MIN_BID_UNSPECIFIED');assert.equal(saved.products[0].status,'unsupported_bid_unit');assert.equal(saved.products[0].product_id,'42');assert.deepEqual(saved.statistics[0],{campaign_id:'7',sku:'101',stat_date:'2026-09-23',impressions:'100',clicks:'10',orders:'11',spend:'25.50',revenue:'200',order_basis:'ATTRIBUTED_ORDERS',complete:true});assert.deepEqual(f.calls.find(row=>row[0]==='stats')[1],{campaignIds:['7'],from:'2026-09-23',to:'2026-09-23'})});
test('an unavailable statistics day fails closed without committing a partial snapshot',async()=>{const f=fixture({transport:{getSkuStatistics:async()=>{throw Object.assign(Error('bad date'),{code:'UPSTREAM_ERROR',status:400})}}});await assert.rejects(f.api.refresh({storeId:'1',expectedRevision:'0',commandId:COMMAND}),{code:'PERFORMANCE_UNAVAILABLE'});assert.equal(f.calls.some(row=>row[0]==='commit'),false);assert.equal(f.calls.filter(row=>row[0]==='fail').length,1)});
test('an empty successful daily response stores coverage without synthetic zero rows',async()=>{const f=fixture({transport:{getSkuStatistics:async(_store,input)=>{f.calls.push(['stats',input]);return[]}}});await f.api.refresh({storeId:'1',expectedRevision:'0',commandId:COMMAND});const saved=f.calls.find(row=>row[0]==='commit')[1];assert.deepEqual(saved.statistics,[]);assert.deepEqual(saved.statisticsCoverage,[{date:'2026-09-23',status:'complete'}]);assert.deepEqual(f.calls.find(row=>row[0]==='stats')[1],{campaignIds:['7'],from:'2026-09-23',to:'2026-09-23'})});
...
test('incomplete, impossible, foreign and invalid-date statistics cannot replace a complete snapshot', async () => {
  ...
  for(const patch of [{expense:null},{clicks:'101'},{campaignId:'999'},{date:'2026-09-31'},{date:'2026-09-24'}]){
