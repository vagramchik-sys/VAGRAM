...
function buildReport({stores,state,current,ozon,date:targetDay}){
 const ozonNames=new Set();
 if(current.active){
...
function create({privateDir,productTypes,now=()=>Date.now()}){
 ...
 function report({stores,categories,date:targetDay=day(Date.now())}){
   const current=classification(stores,categories),ozon=captureOzon(stores,categories,targetDay,current),state=read();
   return buildReport({stores,state,current,ozon,date:targetDay});
 }
 return {report,captureOzon,read};
}
module.exports={create,buildReport,broadCategory,canonicalCategory,treeIndex,productIndex,hierarchyIndex,descendants,withParents,ozonTotals,wbSeries,UNMATCHED,UNMATCHED_ID,day};
